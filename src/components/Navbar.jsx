...
const Navbar = () => {
  ...
  return (
    <header className="fixed w-full z-[100] font-sans">
      {/* 1. TOP UTILITY BAR */}
      <div className={`hidden lg:block transition-all duration-500 bg-slate-900 text-slate-300 ${scrolled ? 'h-0 opacity-0 overflow-hidden' : 'h-10 opacity-100'}`}>
        <div className="max-w-[1440px] mx-auto px-8 h-full flex justify-between items-center text-[11px] font-medium tracking-widest uppercase">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="relative" ref={langRef}>
              <div onClick={() => setShowLangDropdown(!showLangDropdown)} className="flex items-center gap-2 cursor-pointer group hover:text-white transition-all py-1">
                <Globe size={12} className="text-yellow-400 group-hover:rotate-180 transition-transform duration-700" />
                <span className="font-bold">{currentLang.flag} {currentLang.name}</span>
                <ChevronDown size={10} className={`transition-transform ${showLangDropdown ? 'rotate-180' : ''}`} />
              </div>
              <AnimatePresence>
                {showLangDropdown && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-2 z-[110] grid grid-cols-2 gap-1">
                    {languages.map((lang) => (
                      <button key={lang.code} onClick={() => handleTranslate(lang)} className="flex items-center gap-2 px-3 py-2 text-[9px] font-black text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-xl transition-all uppercase">
      ...
      <nav className={`transition-all duration-500 border-b ${
        scrolled 
        ? 'bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl py-3 shadow-lg border-transparent' 
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-2">
            <ul className="flex items-center bg-slate-100/80 dark:bg-slate-800/50 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-700/50">
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.name}>
                    <Link to={item.path} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[12px] font-black uppercase tracking-wider transition-all relative ${
                      isActive ? 'text-slate-900' : 'text-slate-600 dark:text-slate-300 hover:text-blue-600'
                    }`}>
                      {isActive && (
            ...
            <div className="flex items-center gap-3 ml-4">
              <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-yellow-400 border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all">
                {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
              </button>

              {user ? (
                <div className="relative" ref={dropdownRef}>
                  <button onClick={() => setShowUserDropdown(!showUserDropdown)} className="flex items-center gap-3 p-1.5 pr-4 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all text-slate-900 dark:text-white">
                  ...
                  <AnimatePresence>
                    {showUserDropdown && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 p-3 z-[110]">
                ...
                <button onClick={() => setShowJoinModal(true)} className="px-8 py-3 bg-slate-900 dark:bg-blue-600 text-white text-[12px] font-black uppercase tracking-[0.2em] rounded-xl hover:shadow-xl transition-all active:scale-95">
                  Join Us
                </button>
              )}
            </div>
          ...
          <div className="lg:hidden flex items-center gap-3">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 text-slate-700 dark:text-yellow-400">
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed inset-y-0 right-0 z-[150] w-full max-w-sm bg-white dark:bg-slate-950 flex flex-col p-8 shadow-2xl">
              <div className="flex justify-between items-center mb-12">
                <img src={logoImg} alt="Logo" className="h-10" />
                <button onClick={() => setIsOpen(false)} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-900 dark:text-white"><X size={28}/></button>
              </div>
              ...
              <div className="mt-auto pt-10 border-t border-slate-100 dark:border-slate-800">
                {user ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl text-slate-900 dark:text-white">
                  ...
                  <button onClick={() => {navigate('/login'); setShowJoinModal(false)}} className="group flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-900 dark:hover:bg-blue-600 rounded-[2rem] transition-all duration-300">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-left text-slate-900 dark:text-white">
                        ...
                        <p className="text-[10px] font-bold uppercase text-slate-500 group-hover:text-white/60">Access your portal</p>
                      </div>
                    </div>
                  ...
                  <button onClick={() => {navigate('/register'); setShowJoinModal(false)}} className="group flex items-center justify-between p-6 border-2 border-slate-100 dark:border-slate-800 hover:border-yellow-400 rounded-[2rem] transition-all duration-300">
                    <div className="flex items-center gap-6">
                      <div className="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center group-hover:bg-yellow-400 transition-colors">
                        <UserPlus className="text-slate-700 dark:text-slate-300 group-hover:text-slate-900" size={28} />
                      </div>
                      <div className="text-left text-slate-900 dark:text-white">
                        <h4 className="font-black text-lg uppercase tracking-tight group-hover:text-yellow-400 transition-colors">Create Account</h4>
                        <p className="text-[10px] font-bold uppercase text-slate-500">Join our network</p>
                      </div>
                    </div>
