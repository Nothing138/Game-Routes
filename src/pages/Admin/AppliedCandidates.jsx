...
const AppliedCandidates = () => {
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            // আপনার SQL অনুযায়ী এখানে URL চেক করুন
    ...
    const getStatusColor = (status) => {
        switch(status) {
            ...
            case 'applied': return 'bg-yellow-100 text-yellow-600 border-yellow-200';
            case 'interview': return 'bg-purple-100 text-purple-600 border-purple-200';
            default: return 'bg-gray-100 text-gray-500 border-gray-200';
        }
    };

    if (loading) return <div className="p-10 text-center font-bold">Loading Candidates...</div>;
            ...
            <div className="grid grid-cols-1 gap-4">
                {currentItems.length > 0 ? currentItems.map(app => (
                    <div key={app.id} className="group relative">
                        ...
                        <div className="relative bg-white border-2 border-black rounded-[30px] p-6 flex flex-col lg:flex-row items-center justify-between gap-6 overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 w-full lg:w-auto justify-end">
                                <a href={`http://localhost:5000${app.resume_url}`} target="_blank" rel="noreferrer" className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-black hover:text-white transition-all shadow-sm">
                                    <FileText size={16}/>
                                </a>
                                
                                <div className="h-8 w-[1px] bg-gray-100 hidden lg:block mx-2"></div>
                                <button onClick={() => updateStatus(app.id, 'hired')} title="Hire" className="p-3 bg-green-50 text-green-500 rounded-xl hover:bg-green-500 hover:text-white transition-all shadow-sm">
                                    <CheckCircle size={16}/>
                                </button>
                                <button 
                                    onClick={() => updateStatus(app.id, 'interview')} 
                                    title="Call for Interview" 
                                    className="p-3 bg-purple-50 text-purple-500 rounded-xl hover:bg-purple-600 hover:text-white transition-all shadow-sm"
                                >
                                    <Mail size={16}/>
                                </button>
                                <button onClick={() => updateStatus(app.id, 'shortlisted')} title="Shortlist" className="p-3 bg-blue-50 text-blue-500 rounded-xl hover:bg-blue-500 hover:text-white transition-all shadow-sm">
                                    <UserCheck size={16}/>
                                </button>
                                <button onClick={() => updateStatus(app.id, 'rejected')} title="Reject" className="p-3 bg-orange-50 text-orange-400 rounded-xl hover:bg-orange-500 hover:text-white transition-all shadow-sm">
                                    <XCircle size={16}/>
                                </button>
                                <button onClick={() => handleDelete(app.id)} title="Delete" className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm">
                                    <Trash2 size={16}/>
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="text-center py-20 bg-gray-50 rounded-[30px] border-2 border-dashed border-gray-300">
                        <p className="font-bold text-gray-400 uppercase italic">No candidates found for this selection</p>
                    </div>
                )}
            </div>
